<script>
    ...
    onMount(async () => {
        try {
            ...
            steps = recipe.steps.length
                ? recipe.steps.map((s) => ({
                    text: s.text,
                    minutes: s.timer_seconds ? s.timer_seconds / 60 : ''
                }))
                : [{ text: '', minutes: '' }];
    ...
    async function handleSubmit() {
        ...
        try {
            const result = await fetchPatch(`/api/recipes/${id}`, {
                title: title.trim(),
            ...
            toast.error(error?.data?.errorMessage ?? 'Could not update recipe');
        }
    }
...
<main class="recipe-page">
    ...
    {#if loading}
        <p>Loading recipe...</p>
    {:else}
    <h2 class="modal-title">Edit recipe test123</h2>
